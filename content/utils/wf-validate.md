...
Two details make the gating robust:
...
- The gate blocks with `stopImmediatePropagation()` at document capture. Another
  capture-phase document listener registered **earlier** would still run first — load this
  script early.
- Hidden-field skipping uses `getClientRects()`, i.e. actual rendering — a field inside a

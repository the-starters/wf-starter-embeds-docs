---
title: "Step Flow"
source: global-embeds/step-flow
---
...
subflows), per-step footer button groups, a hub/reset-panel pattern, opt-in per-step
validation that disables Continue until the current step's required fields pass, and opt-in
scroll-to-top that pulls the flow back to the top of view on every step change.
...
companion [Panel Nav Flow](./panel-nav-flow.md) script swaps whole panels
...
If you also use [Panel Nav Flow](./panel-nav-flow.md), load that script
alongside it.

## Flow map

Everything in this map is one page of plain Webflow markup (a hub, a linear wizard, and a
branching flow), and every arrow is driven by an attribute, with no custom JS. Edge colors match
the engine's actions: `next`, `branch`, dashed `back` (history-aware), dashed `reset`
(returns to the hub), and `panel-nav` (opening a flow from the hub, via
[Panel Nav Flow](./panel-nav-flow.md)). Walk it with the buttons under
the chart; they are the same buttons the real markup would have:

<StepFlowChart />

## Markup contract

A linear flow has content steps in DOM order plus one shared footer with a button group per step:
...
Click through it below. The left pane is the rendered flow, and the right pane shows which
elements of the markup above the engine is currently showing or hiding:

<StepFlowDemo variant="linear" />

A multi-sub flow uses `data-form-flow-type="multi-sub"`, a radio gate in a `data-form-flow-entry`
...
Same demo for the branching version: pick a reason and the radio gate routes you into
that subflow (Continue with nothing selected reproduces the `console.warn`):

<StepFlowDemo variant="multi-sub" />

### Shared tail steps (multi-sub)

When every branch of a multi-sub flow ends on the same screen — a review + submit step — put that
step **at flow-root level, outside every `data-form-flow-subflow` wrapper**, and point each
branch's last button at it with `data-form-flow-target`. One tail serves all branches instead of
the same step being duplicated inside each of them.

This is a **topology, not a new attribute set**: it uses `data-form-flow-step` /
`data-form-flow-element` and `data-form-flow-target` exactly as documented above.

```html
<div data-form-flow="cancel-membership" data-form-flow-type="multi-sub">
  <div data-form-flow-entry data-form-flow-element="step-1">…radio gate…</div>

  <div data-form-flow-subflow data-form-flow-element="step-2a">
    <div data-form-flow-step data-form-flow-element="step-2a-1">…</div>
    <div data-form-flow-step data-form-flow-element="step-2a-2">…</div>
    <div data-form-flow-button-group="step-2a-1">
      <button type="button" data-form-flow-action="next">Continue</button>
      <button type="button" data-form-flow-action="back">Go Back</button>
    </div>
    <div data-form-flow-button-group="step-2a-2">
      <!-- leaves the branch for the root-level tail -->
      <button type="button" data-form-flow-action="next" data-form-flow-target="step-3">Continue</button>
      <button type="button" data-form-flow-action="back">Go Back</button>
    </div>
  </div>

  <div data-form-flow-subflow data-form-flow-element="step-2b">
    <div data-form-flow-step data-form-flow-element="step-2b-1">…</div>
    <div data-form-flow-button-group="step-2b-1">
      <button type="button" data-form-flow-action="next" data-form-flow-target="step-3">Continue</button>
    </div>
  </div>

  <!-- The shared tail: a root-level step, outside every subflow wrapper. -->
  <div data-form-flow-step data-form-flow-element="step-3">…review…</div>
  <div data-form-flow-button-group="step-3">
    <button type="button" data-form-flow-action="back">Go Back</button>
...
- **Forward.** A `next` whose `data-form-flow-target` is **not** inside the active branch but
  **does** exist at flow-root level widens the scope to the root and drops the branch: the branch
  wrapper is hidden and the tail step shows with its own button group. A target that is inside the
  branch, or that exists nowhere, keeps the old behaviour (branch scope, then a `console.warn`
  naming the step). Without this widening the lookup would miss and every content step would stay
  hidden — a blank panel.
- **A plain `next` inside a branch is unaffected.** With an active branch the scope is that
  branch's own steps, so the root-level tail is not in scope and Continue still walks the branch in
  DOM order.
- **Back.** History stores the **wrapper element**, not just a step id, so `back` from the tail
  restores the exact branch the member came from, and backing out further walks that branch's steps
  and finally returns to the entry gate.

**Multiple tails are supported, and each one is terminal.** Two branches can funnel into `step-3`
while a third funnels into `step-3a`; both are alternative endpoints, never a sequence. With no
active branch a multi-sub flow reports **no** next step, so every tail's `type="submit"` button
falls through to Webflow's native submit even when another tail sits later in the DOM. (This gate
applies only to multi-sub flows — root-level steps chaining in DOM order is a linear flow's core
mechanism and still works there, with only its last step's submit terminal.)

An optional hub holds several flows inside `data-form-flow="main-container"` with a
...
### Scroll to top (opt-in)

Active only when the flow root has `data-form-flow-scroll-top="true"` (case-insensitive, so
Webflow's `"True"` works). Every step change then pulls the flow root back to the top of view,
whether that step change came from `next`, `back`, a branch, a `reset` to step-1, or a programmatic
`goTo`. All three attributes live on the `[data-form-flow]` root, so each flow on the page carries
its own scroll settings.

| Attribute | Where | Purpose |
| --- | --- | --- |
| `data-form-flow-scroll-top="true"` | Flow root | Turns scroll-to-top on for the flow. Omit it (or set anything else) and nothing scrolls. |
| `data-form-flow-scroll-offset="<px>"` | Flow root | Extra pixels of breathing room above the root, e.g. `40`. Defaults to `0`. |
| `data-form-flow-scroll-container="window"` | Flow root | Forces the page scroller instead of auto-detecting one. |
| `data-form-flow-scroll-container="nearest"` | Flow root | Same as leaving the attribute off: use the nearest scrollable ancestor, else the window. |

Scroll rules found in the code:

- The scroll only ever goes **up**. If the top of the flow root is already visible the computed
  target sits at or below the current position, and the routine does nothing, so it never fights a
  visitor who scrolled somewhere on purpose.
- It never fires on initial page load. Scrolling arms itself only after hydration has painted
  step-1, so the first real transition is the earliest thing that can scroll.
- With no `data-form-flow-scroll-container`, the scroller is auto-detected: the nearest ancestor
  that both scrolls its own overflow (`overflow-y: auto` or `scroll`) *and* currently overflows,
  otherwise the window. A pane styled `overflow-y: auto` whose content fits does not count.
- Window scrolls clear sticky and fixed navbar chrome. Bars tagged `data-toc-navbar` win (the same
  site-wide tag the section custom TOC embed uses, and several tagged bars sum their heights), with
  Webflow's `.w-nav` as the fallback when nothing is tagged. Position and height are measured live
  at scroll time, so a bar that is `fixed` on desktop and static on mobile, or hidden at one
  breakpoint, needs no per-breakpoint config.
- A scroll inside a scrollable ancestor skips the navbar offset, since page chrome cannot cover a
  panel, but `data-form-flow-scroll-offset` still applies there.
- Scrolling is smooth, and instant for visitors who ask for `prefers-reduced-motion: reduce`.
- `data-form-flow-scroll-offset` is Webflow-tolerant: a trailing unit (`"40px"`) still reads as 40,
  and a value that is unparseable or negative reads as `0`.
...
- **Terminal submit passes through.** A real `type="submit"` button with no explicit form-flow
  ...
  `data-form-flow-action="next"`. In a multi-sub flow every [shared tail](#shared-tail-steps-multi-sub)
  submit is terminal, because a flow with no active branch reports no next step.
...
- **Accessibility.** Hidden steps, groups, and flows get `aria-hidden="true"`; shown elements get
  ...
  the wrapper and the inner button/link. There is no focus management on step change, so focus stays
  where it was.
- **Scroll-to-top fails quietly.** The scroll measurement is wrapped in a try/catch, so a bad
  measurement can never break navigation and nothing is logged. If a step change does not scroll,
  check the `data-form-flow-scroll-top` value first, then whether the top of the flow root was
  already in view (the scroll is up-only, and a visible top is left alone).

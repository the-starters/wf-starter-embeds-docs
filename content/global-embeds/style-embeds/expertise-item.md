...
It is pure CSS: the Webflow structure supplies the elements, and this embed supplies the
transitions and the hover states.
...
On hover (only under `@media (hover: hover) and (pointer: fine)`, meaning real mice only, with no
touch or stylus emulation):
...
- The icon swap animates `filter: blur()` and `scale`/`translate`, which is cheap on modern
  browsers, but avoid adding more blurred layers per card.

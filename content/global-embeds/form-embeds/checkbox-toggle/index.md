...
Shows or hides an element depending on whether a checkbox is checked. Plain IIFE — no GSAP, no
...
`input type="checkbox"` anywhere in the wrapper — so the `checkbox` slot is optional, the `item`
...
- If the JS never loads, the item stays hidden forever (the pre-init rule has no fallback) — keep

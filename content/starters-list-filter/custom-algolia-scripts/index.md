...
- **[Disable Apply](./disable-apply.md)** (`disable-apply.js`): keeps an Apply button visually and functionally
  disabled until the filter groups it commits actually have a selection.
- **[Filter Visibility Empty](./filter-visibility-empty.md)** (`filter-visibility-empty.js`): opt-out attribute that keeps an
  element visible when WF-Algolia inline-hides it for being empty.
- **[Filters Text](./filters-text.md)** (`filters-text.js`): humanizes raw facet keys and values in filter chips
  and selected-value slots ("also-worked-with" → "Company").
- **[Range Backfill Rate](./range-backfill-rate.md)** (`range-backfill-rate.js`): restores an emptied range input to its
  authored bound so WF-Algolia always reads two numbers.
- **[Scroll Filter](./scroll-filter.md)** (`scroll-filter.js`): scrolls an opted-in filter option list back to the
  top after a selection re-sorts it.
- **[Clear Filter Visibility](./clear-filter-visibility/index.md)** (`clear-filter-visibility/`): makes the reset button clear all
